...
function GuildRank() {
    ...
    async function getGuildInfo() {
        try {
            const response = await axios.get(`https://developer-lostark.game.onstove.com/armories/characters/존중이리퍼/cards`, {
                headers: {Authorization: `bearer ${Key}`}
            })
            console.log(response);
            
            return setServerName(response.data)
    ...
    return (
        <div>
            <img src={ServerName[0]} alt="11111" />
            <h1>aa</h1>
        </div>
    )
}
